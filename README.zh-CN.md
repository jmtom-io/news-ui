...
- 装了信息资讯类 skill（如follow-builders @zarazhang 等），想要一个易读的阅读界面
- 希望把多个agent推送的推文、播客、文章统一汇聚成一份"报纸"来读
- 想要留存历史记录、按日期或来源搜索内容
...
1. **易读排版**：推文、播客、文章，新闻资讯统一以报纸风格呈现
2. **多来源聚合**：支持同时接入多个信息 skill，来源分类展示
3. **侧边栏筛选**：可按日期或skill来源一键切换
5. **全文搜索**：跨历史记录搜索关键词
6. **本地运行**：所有数据存在本机 `~/.news-ui/history/`，不上传任何服务器
7. **开箱即用**：浏览器直接打开 http://localhost:3000 即可阅读
8. **自动更新**：每次运行已连接的信息 skill，网页会自动刷新内容
